...
approval: 미확인          # question·plan 통과에 필요. 승인 시 "<stage> 승인 YYYY-MM-DD" (예: plan 승인 2026-10-01)
created: 2026-09-07
...
- 정확 중복은 같은 폴더 안 같은 해시. signals는 같은 organization까지 같아야 한다. 문구가 바뀐 중복은 Claude가 판단해 `--relation repost`로 잇는다.
...
| EXP-*.md | 머리글 id 불일치, kind·status 값, 게이트 위반 | skip으로 덮인 게이트 위반, completed인데 실제 관찰 절이 비어 있음 |
...
`unmet`으로 세는 통과 조건(status가 active인 프로젝트에서만 계산): reading은 `reading.md`의 LIB 링크 수 ≥ `min_library`, reproduction은 reproduction 실험 status completed/blocked, pilot은 pilot 실험 completed + verdict, question·plan은 `approval:` 값이 해당 단계 이름을 포함(예: plan 단계는 `plan 승인 ...`). 나머지 통과 조건은 Claude가 판단한다.
